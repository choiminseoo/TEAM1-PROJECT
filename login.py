...
def register_window():
    global reg_username_entry, reg_password_entry, reg_name_entry, reg_age_entry, reg_likeGenre_combo, reg_haveOtt_combo, gender_var, register_window

    register_window = tk.Toplevel(root)
    register_window.title("회원가입")

    # 아이디 입력 레이블 및 엔트리 위젯
    reg_username_label = tk.Label(register_window, text="아이디:")
    ...
    reg_password_label = tk.Label(register_window, text="비밀번호:")
    reg_password_label.grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
    reg_password_entry = tk.Entry(register_window, show="*")  # 입력 내용을 마스킹합니다.
    reg_password_entry.grid(row=1, column=1, padx=5, pady=5)


    reg_name_label = tk.Label(register_window, text="이름:")
    ...
    gender_label = tk.Label(register_window, text="성별:")
    gender_label.grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)
    gender_var = tk.StringVar(value="남자")
    male_radio = tk.Radiobutton(register_window, text="남자", variable=gender_var, value="남자")
    male_radio.grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)
    female_radio = tk.Radiobutton(register_window, text="여자", variable=gender_var, value="여자")
    female_radio.grid(row=3, column=2, padx=5, pady=5, sticky=tk.W)

    reg_age_label = tk.Label(register_window, text="나이:")
    reg_age_label.grid(row=4, column=0, padx=5, pady=5, sticky=tk.W)
    reg_age_entry = tk.Entry(register_window)
